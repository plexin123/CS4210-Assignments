...
for val in n: #iterates over n
    ...
    for ra in r: #iterates over r
        ...
        for algo in algorithms: #iterates over the algorithms
            ...
            if algo == Perceptron:
                clf = Perceptron(eta0= val, shuffle=ra , max_iter= 1000)
            else:
                clf = MLPClassifier(activation='logistic', learning_rate_init=val, hidden_layer_sizes=(25,), shuffle=ra, max_iter=1000)
            #if Perceptron then
            ...
            accuracy = clf.score(X_test, y_test)
                # print(f"this is {accuracy}")
            if accuracy > max:
                max = accuracy
                print(f"Highest {algo} accuracy so far: {max}, Parameters:{val} and {ra}")
